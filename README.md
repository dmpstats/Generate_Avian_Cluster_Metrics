...
Attributes listed in the table below correspond to column names in the
whole-cluster metrics dataset generated by the App. Each row represents
the properties of a cluster event, which may include one or multiple
tracks at any point during the cluster’s lifetime. Cluster IDs are
provided in the input data column specified by the `cluster_id_col`
parameter. How these metrics appear in the App’s output is determined by
the `output_type` and `cluster_tbl_type` settings:
